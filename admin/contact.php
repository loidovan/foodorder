...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete'])) {
    if ($str != '') {
        $del_check = $contact->delete_contact($str);
?>
        <script>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['allow'])) {
    if ($str != '') {
        $allow_check = $contact->allow_contact($str);
    ?>
        <script>
            if (window.history.replaceState) {
                window.history.replaceState(null, null, window.location.href);
                    ...
                    <form action="" method="post">
                        <div class="card mb-4">
                            <div class="card-header">
                                ...
                                <button type="submit" name="delete" class="btn btn-primary btn-sm ml-2" onclick="return confirm('Bạn chắc chắn muốn xóa các mục đã chọn?')" style="float: right;">Xóa liên hệ</button>
                                <button type="submit" name="allow" class="btn btn-primary btn-sm" style="float: right;width:104px">Hợp lệ</button>
                                <a href="contactExcel.php" class="btn btn-primary btn-sm mr-2" style="float:right">Xuất Excel <i class="fas fa-file-excel"></i></a>
